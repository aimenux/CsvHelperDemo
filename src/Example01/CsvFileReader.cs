...
public interface ICsvFileReader
{
    ...
    Task<ICollection<TU>> GetRecordsAsync<TU, TV>(string file, CancellationToken cancellationToken) where TV : ClassMap<TU>;
}
...
public class CsvFileReader : ICsvFileReader
{
    ...
    public async Task<ICollection<TU>> GetRecordsAsync<TU, TV>(string file, CancellationToken cancellationToken) where TV : ClassMap<TU>
    {
        using var reader = new StreamReader(file);
        using var csv = new CsvReader(reader, _csvConfiguration);
        csv.Context.RegisterClassMap<TV>();
        var records = csv.GetRecordsAsync<TU>(cancellationToken);
        return await records.ToListAsync(cancellationToken);
    }
}